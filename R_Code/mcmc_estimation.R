...
inits <- function () {
  list(beta.raw = rnorm(ncol(X), 0, 1),
       ...
       xi.beta = runif(1, 0, 1),
       xi.gamma.cons = runif(1, 0, 1),
       xi.gamma.edu = runif(1, 0, 1),
       xi.gamma.fem = runif(1, 0, 1)
  )
}
...
parameters <- c("sigma.alpha", "beta",
                ...
                "xi.beta", "xi.gamma.cons", "xi.gamma.edu", "xi.gamma.fem")

# Estimate the JAGS model (and print the run time).
print(system.time(
  m <- jags.parallel(data, inits, parameters, model = "BUGS/model.bugs", 
                     n.chains = 10, n.iter = 5000, DIC = FALSE, n.thin = 2)
))

# Save the simulations.
save(m, file = "R_Images/mcmc_sims.RData")

pdf("HPC/converge.pdf")
plot(m)
dev.off()
