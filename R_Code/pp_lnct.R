...
f.lnct <- (c(min(d$lnct), max(d$lnct)) - mean(d$lnct))/(2*sd(d$lnct))
...
pp.lnct <- list(p.hi.stat.gen = p.hi.stat.gen, p.lo.stat.gen = p.lo.stat.gen, fd.stat.gen = fd.stat.gen, rr.stat.gen = rr.stat.gen, or.stat.gen = or.stat.gen,
...
save(pp.lnct, file = "R_Images/pp_lnct.RData")