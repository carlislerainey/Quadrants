...
# graphics parameters 
# png("Manuscript/Hypothesis_Tests/Figures/fem_stat_gen.png", height = 250, width = 600)
...
fd <- round(quantile(pp.fem.stat.gen$fd.ngend, 0.5), 2)
lwr <- round(quantile(pp.fem.stat.gen$fd.ngend, 0.05), 2)
upr <- round(quantile(pp.fem.stat.gen$fd.ngend, 0.95), 2)
...
p <- plogis(y.star)
lines(f.fem, p, col = "black", lwd = 3)
fd <- round(quantile(pp.fem.stat.gen$fd.gend, 0.5), 2)
lwr <- round(quantile(pp.fem.stat.gen$fd.gend, 0.05), 2)
upr <- round(quantile(pp.fem.stat.gen$fd.gend, 0.95), 2)
text(mean(xat0), .975, paste("FD = ", 
                             sprintf("%.2f", fd, 2), 
                             " [",
                             sprintf("%.2f", lwr, 2),
